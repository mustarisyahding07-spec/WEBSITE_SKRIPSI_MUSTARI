    ...
    <div class="container mx-auto px-6">
        ...
        @if(session('cart') && count(session('cart')) > 0)
            <div class="flex flex-col lg:flex-row gap-12 max-w-7xl mx-auto">
                ...
                <!-- Checkout Form -->
                <div class="lg:w-1/3" x-data="checkoutForm()">
                    <div class="bg-white rounded-3xl shadow-xl border border-gray-100 p-8 sticky top-28">
                        ...
                        <div class="space-y-4 mb-8">
                            ...
                            <div class="flex justify-between text-gray-600" x-show="shippingCost > 0" x-cloak>
                                <span>Ongkir (<span x-text="courierName"></span>)</span>
                                <span x-text="'Rp ' + formatNumber(shippingCost)"></span>
                            </div>
                        ...
                        <form action="{{ route('cart.checkout') }}" method="POST" class="space-y-5">
                            ...
                            <div>
                                <label for="phone" class="block text-sm font-bold text-gray-700 mb-2">No. WhatsApp <span class="text-red-500">*</span></label>
                            ...
                            <div class="border-t border-gray-100 pt-5">
                                <h3 class="font-bold text-gray-900 mb-4">📍 Lokasi Pengiriman</h3>
                                @php
                                    $totalWeight = 0;
                                    foreach(session('cart', []) as $item) {
                                        $totalWeight += ($item['weight'] ?? 250) * $item['quantity'];
                                    }
                                    $totalWeight = max($totalWeight, 1000); // Minimum 1kg
                                @endphp
                                <x-location-picker :totalWeight="$totalWeight" />
                            </div>
                            ...
                            <button 
                                type="submit" 
                                class="w-full bg-gray-900 hover:bg-black text-white font-bold rounded-xl text-lg px-5 py-4 text-center inline-flex items-center justify-center gap-3 transition-colors duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed"
                            ...
                            >
                                ...
                                <span x-text="shippingCost > 0 ? 'Pesan via WhatsApp' : 'Pilih lokasi dulu'"></span>
                            </button>
                            
                            <p x-show="shippingCost <= 0" class="text-center text-sm text-amber-600">
                                ⚠️ Pilih lokasi pengiriman dan kurir terlebih dahulu
                            </p>
                        </form>
                    </div>
                </div>
...
function checkoutForm() {
    return {
        ...
        init() {
            // Listen for shipping-selected event from location-picker component
            this.$el.addEventListener('shipping-selected', (e) => {
                this.shippingCost = e.detail.cost;
                this.courierName = e.detail.courier + ' ' + e.detail.service;
            });
        },
